...
function ArmyViewer({armyId, onUpdatePosition, onCityCreated}) {
    ...
    const submitCoordinates = async () => {
        ...
        try {
            ...
            await axios.post(`${process.env.REACT_APP_BACKEND_PATH}/army/armies/${armyId}/update-coordinates`, {
                x: newX,
                y: newY
            });
    ...
    return (
        <Draggable>
            <div className="bg-gray-600 border-4" style={{
                padding: "1rem",
                zIndex: 1,
                position: 'absolute',
                top: '10%',
                ...
                <TreeView aria-label="file system navigator">
                    ...
                    {showInputFields && (
                        <div>
                            <TextField label="X Coordinate" variant="outlined" name="x" value={coordinates.x}
                                       onChange={handleInputChange} sx={{margin: "10px"}}/>
                            <TextField label="Y Coordinate" variant="outlined" name="y" value={coordinates.y}
                                       onChange={handleInputChange} sx={{margin: "10px"}}/>
                    ...
                    <TreeItem className="border-2" sx={{padding: "0.25rem"}} nodeId={`stats-${armyId}`}
                              label={`stats`}></TreeItem>
                    <TreeItem className="border-2" sx={{padding: "0.25rem"}} nodeId={`total-${armyId}`}
