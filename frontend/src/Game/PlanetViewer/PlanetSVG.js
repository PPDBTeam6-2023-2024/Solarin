        ...
        <>
            <svg onPointerMove={(e) =>
                setMousePos([e.pageX / props.screenSize.current?.clientWidth,
                 ...
                 style={{pointerEvents: "none", width: "100vw", height: "auto", position: "absolute", zIndex: 16}} viewBox={'0 0 ' + width + ' ' + height}
                 preserveAspectRatio="none">
                {/* For the help lines when in move mode*/}
                {
                    props.armiesMoveMode.map((army, i) => {
                        ...
                        return <Fragment key={army.id}>
                            <line stroke={"red"} strokeWidth={3} key={i} x1={armyImage.curr_x * width}
                                  y1={armyImage.curr_y * height} x2={mousePos[0] * width} y2={mousePos[1] * height}/>
                            <circle cx={mousePos[0] * width} cy={mousePos[1] * height} r={10} fill={"red"}/>
                            <text fill="white" x={(mousePos[0] + armyImage.curr_x) * width / 2}
                                  y={(mousePos[1] + armyImage.curr_y) * height / 2 - 20}>
