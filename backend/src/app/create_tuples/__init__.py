...
async def create_planet_types(dev: DeveloperAccess, planet_types: list[dict[str, Any]]):
    for planet_type in planet_types:
        await dev.createPlanetType(planet_type["type-name"], planet_type["description"])


async def create_region_types(dev: DeveloperAccess, region_types: list[dict[str, Any]]):
    for region_type in region_types:
        await dev.createPlanetRegionType(region_type["type-name"], region_type["description"])


async def create_building_types(dev: DeveloperAccess, building_types: list[dict[str, Any]]):
...
async def create_resource_types(dev: DeveloperAccess, resource_types: list[str]):
    for resource_type in resource_types:
        await dev.createResourceType(resource_type)


async def create_troop_types(dev: DeveloperAccess, troop_types: list[dict[str, Any]]):
...
async def create_tuples():
    async with sessionmanager.session() as session:
        dev = DeveloperAccess(session)
        types_file = open(f"{os.path.dirname(os.path.abspath(__file__))}/types.json").read()
        types = json.loads(types_file)
        await create_resource_types(dev, types["resources"])
        await create_region_types(dev, types["regions"])
        await create_planet_types(dev, types["planets"])
        await create_troop_types(dev, types["units"])
        await create_building_types(dev, types["buildings"])
